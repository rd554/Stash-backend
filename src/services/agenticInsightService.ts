...
export class AgenticInsightService {
  ...
  private initializeInsightTemplates() {
    this.insightTemplates = [
      ...
      // Financial Health Analytics Insight
      {
        type: 'financial_health_analytics',
        priority: 'info',
        title: 'Financial Health Analytics',
        content: 'Your financial health score analysis',
        conditions: (transactions, userProfile) => { return true; },
        generateContent: async (transactions, userProfile) => {
          console.log('🔍 HEALTH SCORE DEBUG: Starting calculation...');
          console.log('🔍 HEALTH SCORE DEBUG: User salary:', userProfile.salary);
          console.log('🔍 HEALTH SCORE DEBUG: User spending personality:', userProfile.spendingPersonality);
          console.log('🔍 HEALTH SCORE DEBUG: Total transactions received:', transactions.length);
          console.log('🔍 HEALTH SCORE DEBUG: All transactions:', transactions.map(t => ({
            category: t.category, amount: t.amount, date: t.date, merchant: t.merchant
          })));

          // Use all transactions (persona + manual) up to today
          const today = new Date();
          const allUpToToday = transactions.filter(t => new Date(t.date) <= today);
          
          console.log('🔍 HEALTH SCORE DEBUG: Total transactions up to today:', allUpToToday.length);
          console.log('🔍 HEALTH SCORE DEBUG: Sample transactions:', allUpToToday.slice(0, 5).map(t => ({
            date: t.date,
            category: t.category,
            amount: t.amount,
            merchant: t.merchant
          ...
          const categorySpending = allUpToToday.reduce((acc, t) => {
            const normalizedCategory = this.normalizeCategory(t.category, userProfile.spendingPersonality);
            acc[normalizedCategory] = (acc[normalizedCategory] || 0) + t.amount;
            return acc;
          }, {} as Record<string, number>);
          
          console.log('🔍 HEALTH SCORE DEBUG: All-time category spending:', categorySpending);
          console.log('🔍 HEALTH SCORE DEBUG: User spending personality:', userProfile.spendingPersonality);
          
          // Get budget data
          const budgetData = await this.getBudgetDataForUser(userProfile.spendingPersonality, userProfile.userId);
          if (!budgetData) {
            return 'Unable to calculate health score: missing budget data.';
          }

          // Compare each category to budget cap
          let budgetViolations = 0;
          let totalBudgetViolationAmount = 0;
          ...
          Object.entries(budgetData).forEach(([category, { budgetCap }]) => {
            ...
            if (spent > budgetCap) {
              budgetViolations++;
              totalBudgetViolationAmount += (spent - budgetCap);
              // More aggressive penalty for budget violations
              const overrunPercentage = ((spent - budgetCap) / budgetCap) * 100;
              score = Math.max(0, 100 - Math.round(overrunPercentage * 2)); // Double penalty
            } else {
              // Reward for being under budget, but not 100 if very close to cap
              const underPercent = (budgetCap - spent) / budgetCap;
              score = Math.round(70 + 30 * underPercent); // 70-100 (lower max for under budget)
            }
            ...
            totalScore += score;
            console.log(`🔍 HEALTH SCORE DEBUG: Category ${category} - spent: ${spent}, budget: ${budgetCap}, overrun: ${spent > budgetCap ? ((spent - budgetCap) / budgetCap * 100).toFixed(1) + '%' : '0%'}, score: ${score}`);
          });
          
          // Calculate final score with stronger penalties
          const avgScore = Math.round(totalScore / totalCategories);
          
          // Much stronger penalty for budget violations
          let finalScore = avgScore - (budgetViolations * 20); // 20 points per violation instead of 10
          
          // Additional penalty for total violation amount
          const totalViolationPercentage = (totalBudgetViolationAmount / userProfile.salary) * 100;
          finalScore -= Math.round(totalViolationPercentage * 10); // 10 points per 1% of salary
          
          if (finalScore < 0) finalScore = 0;
          if (finalScore > 100) finalScore = 100;
          
          console.log('🔍 HEALTH SCORE DEBUG: Total budget violations:', budgetViolations);
          console.log('🔍 HEALTH SCORE DEBUG: Total violation amount:', totalBudgetViolationAmount);
          console.log('🔍 HEALTH SCORE DEBUG: Violation percentage of salary:', totalViolationPercentage.toFixed(2) + '%');
          console.log('🔍 HEALTH SCORE DEBUG: Average score before penalties:', avgScore);
          console.log('🔍 HEALTH SCORE DEBUG: Final health score:', finalScore);
          console.log('🔍 HEALTH SCORE DEBUG: Weakest area:', weakestArea, 'Score:', weakestScore);
          
          return `Your financial health score is ${finalScore}/100. Focus on ${weakestArea || 'any category'} (${weakestScore}/100) to improve your overall score.`;
        }
      },
